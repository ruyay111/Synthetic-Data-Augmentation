...
PoolProcessor loads specialist windows, maps them to HMM emission units, and
checks pool length and variance ordering.
"""
...
def load_generated_images(
    ...
    *,
    ...
    if returns is not None:
        ...
        auto_mu, auto_sd = PriceReturnProcessor().A001LogReturnScale(returns)
    ...
    *,
    ...
    for regime in range(n_regimes):
        ...
        if seed is not None:
            ...
            pool = pool[: min(n_windows, pool.shape[0])]
        flat = EqualWeightProcessor().EmissionFromLogPanel(pool, scale).reshape(-1)
        if calibrate_regimes is not None:
            target_mean, target_std = calibrate_regimes[regime]
            flat = affine_calibrate(flat, target_mean, target_std)
        generated[str(regime)] = flat
...
class PoolProcessor:
    """Load, calibrate, and validate sampled specialist pools."""

    def PoolDir(self, pools_root: Path, regime: int) -> Path:
        """
        Directory for one regime's sampled pool.

        Parameters:
        pools_root: pathlib.Path
            Root of all regime pools.
        regime: int
            Regime index.

        Return:
           pathlib.Path of regime_k{regime}.
        """
        return pool_dir(pools_root, regime)

    def LoadPool(self, pools_root: Path, regime: int) -> np.ndarray:
        """
        One regime's pool, shape (n_pool, seq_len, n_channels).
        ...
        Return:
           numpy.ndarray of sampled windows.
        """
        return load_pool(pools_root, regime)

    def LoadPoolMeta(self, pools_root: Path, regime: int) -> dict[str, Any]:
        """
        JSON metadata for one regime pool.
        ...
        Return:
           dict, empty if meta.json is missing.
        """
        return load_pool_meta(pools_root, regime)

    def LoadGeneratedImagesEw(
        self,
        pools_root: Path,
        n_regimes: int,
        scale: dict[str, Any],
        ...
        Parameters:
        pools_root: pathlib.Path
            Root of specialist pools.
        n_regimes: int
            Number of regimes.
        scale: dict
            EW scale from EqualWeightProcessor.EwScale.
        seed: int or None
            Optional subsample seed.
        n_windows: int or None
            Optional cap on windows per regime.
        calibrate_regimes: dict or None
            Per-regime (mean, std) affine targets.

        Return:
           dict mapping regime string to a flat emission array.
        """
        return load_generated_images_ew(
            pools_root,
            n_regimes,
            scale,
            seed=seed,
            n_windows=n_windows,
            calibrate_regimes=calibrate_regimes,
        )
    ...
    def CheckPools(
        ...
        """
        Check pool length, finite values, and variance ordering.

        Parameters:
        generated: dict
            Regime string to flat pool.
        regime_paths: list
            Label paths that will draw from the pools.
        real_series: numpy.ndarray
            Real emissions for variance comparison.
        ...
        Return:
           tuple of (problem strings, per-regime summary dict).
        """
        return check_pools(generated, regime_paths, real_series, real_labels, n_regimes)
