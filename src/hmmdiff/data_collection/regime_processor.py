...
RegimeProcessor bundles Vol_Regime clustering, label cache I/O, and transition
summaries. Standalone helpers below the class remain snake_case utilities.
Literature: Hamilton (1989); Killick, Fearnhead, and Eckley (2012) for PELT.
"""
...
class RegimeProcessor:
    """Volatility-regime clustering and label cache.

    Fits Vol_Regime (GARCH, PELT changepoints, spectral or k-means assignment),
    writes npz caches, and summarizes transitions.
    """
    ...
    def FitRegimes(
        ...
        train_series: numpy.ndarray
            Return series used for clustering (typically EW z-scores).
        cfg: dict
            Pipeline config with a regimes block.
        coverage_end: int or None
            Exclusive index that must contain every regime (HMM train length).

        Return:
           RegimeLabels dataclass.
        """
        return fit_regimes(train_series, cfg, coverage_end=coverage_end)

    def SaveLabels(self, regimes: RegimeLabels, path: Path) -> None:
        """
        Write regime labels to an npz cache.

        Parameters:
        regimes: RegimeLabels
            Fitted labels and metadata.
        path: pathlib.Path
            Output npz path.
        ...
        Return:
           None
        """
        save_labels(regimes, path)

    def LoadLabels(self, path: Path) -> RegimeLabels:
        """
        Load a previously saved npz label cache.
    ...
    def AverageRegimeLength(self, labels: np.ndarray) -> dict[str, float]:
        """
        Mean consecutive days per regime visit.
        ...
        Return:
           dict mapping regime id string to mean length.
        """
        return average_regime_length(labels)

    def Switchpoints(self, labels: np.ndarray) -> list[int]:
        """
        Indices at which the regime changes, with series length appended.
        ...
        Return:
           list of int.
        """
        return switchpoints(labels)

    def EwmVolatility(self, series: np.ndarray, com: float) -> np.ndarray:
        """
        Exponentially weighted volatility of the return series.
        ...
        Return:
           numpy.ndarray of volatility.
        """
        return ewm_volatility(series, com)

    def EmpiricalTransitionMatrix(self, labels: np.ndarray, n_regimes: int) -> np.ndarray:
        """
        Row-normalized transition counts over the label path.
        ...
        Return:
           numpy.ndarray of shape (K, K).
        """
        return empirical_transition_matrix(labels, n_regimes)

    def OutboundTransitionMatrix(
        self, transition_matrix: np.ndarray, n_regimes: int
    ) -> np.ndarray:
        """
        Transition probabilities conditional on leaving the current regime.
        ...
        Return:
           numpy.ndarray of shape (K, K) with zero diagonal.
        """
        return outbound_transition_matrix(transition_matrix, n_regimes)
