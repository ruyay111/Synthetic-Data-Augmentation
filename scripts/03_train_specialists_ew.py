...
Same training flags as ``03_train_specialists.py``. Defaults to ``configs/ew.yaml`` so checkpoints
land under ``checkpoints/ew`` and do not overwrite the A001 specialists.
...
def main() -> int:
    ...
    for regime in regimes:
        ...
        if not data_path.exists():
            print(f"[SKIP] regime {regime}: {data_path} missing; run scripts/02_build_diffusion_dataset_ew.py")
            continue
